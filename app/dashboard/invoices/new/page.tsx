...
function NewInvoiceForm() {
  ...
  function validate(): FormErrors {
    ...
    if (emptyDesc)               e.items = "Todos los conceptos deben tener descripción"
    const zeroAmt  = items.some(i => i.unitPrice <= 0)
    if (zeroAmt)                 e.items = (e.items ? e.items + " · " : "") + "Revisa los precios (deben ser > 0)"
    return e
  }
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/dashboard/invoices" className="btn-icon">
          <ArrowLeft size={16} />
        </Link>
        <div>
          <h1>Nueva factura</h1>
          <p className="text-sm text-text-secondary mt-0.5">El número FAC-YYYY-XXXX se asignará automáticamente</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        ...
        <div className="card space-y-4">
          <p className="section-title">Datos de facturación</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Input
              ...
              hint="Opcional · Aparecerá en el PDF"
            />
          </div>
          <Textarea
            label="Notas y condiciones de pago"
            placeholder="Ej: Pago a 30 días · Transferencia bancaria · IBAN ES00 0000 0000 0000 0000 0000"
            value={notes}
                ...
                <div key={item.id} className="grid grid-cols-12 gap-2 items-center group">
                  ...
                  <div className="col-span-12 sm:col-span-6">
                    <input
                      ...
                      placeholder={`Concepto ${idx + 1} — descripción del servicio`}
